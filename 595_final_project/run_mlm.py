...
from transformers import (CONFIG_MAPPING, MODEL_MAPPING, AdamW, AutoConfig,
                          AutoModelForMaskedLM, AutoTokenizer, SchedulerType,
...
from knowledge_bert.modeling import BertConfig, BertForMaskedLM
...
def parse_args():
    ...
    parser.add_argument(
        "--max_train_samples", type=int, default=None, help="Maximum training samples to use for training."
    )
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if "validation" not in raw_datasets.keys():
            ...
            if args.max_train_samples:
                raw_datasets["validation"] = raw_datasets["validation"].select(range(int(args.max_train_samples * args.validation_split_percentage / 100)))
    ...
    config = BertConfig('ernie_config.json')
    ...
    if args.model_name_or_path:
        model, missed_key = BertForMaskedLM.from_pretrained(args.model_name_or_path, config=config)
    ...
    if args.line_by_line:
        ...
        def tokenize_function(examples):
            ...
            # outputs['input_ents'] = torch.randn(len(examples[text_column_name]), max_seq_length, 100)
            # outputs['ent_mask'] = torch.ones(len(examples[text_column_name]), max_seq_length)
            return outputs
        ...
        def tokenize_function(examples):
            ...
            outputs['input_ents'] = [torch.randn(len(outputs['input_ids'][i]), 100).tolist() for i in range(len(outputs['input_ids']))]
            outputs['ent_mask'] = [torch.ones(len(outputs['input_ids'][i])).tolist() for i in range(len(outputs['input_ids']))]
            return outputs
