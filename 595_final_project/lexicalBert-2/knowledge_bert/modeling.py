...
class BertOutput_simple(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, input_tensor_ent):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class BertLayer(nn.Module):
    def __init__(self, config, num):
        ...
        self.config = config
        self.num = num
        self.layer_types = config.layer_types[num]

        if self.layer_types == 'sim':
            self.attention = BertAttention_simple(config)
            self.intermediate = BertIntermediate_simple(config)
            self.output = BertOutput_simple(config)
        elif self.layer_types == 'mix':
            self.attention = BertAttention_simple(config)
            self.intermediate = BertIntermediate(config)
            self.output = BertOutput(config)
        elif self.layer_types == 'norm':
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config, num) for num in range(config.num_hidden_layers)])
...
class BertForMaskedLM(PreTrainedBertModel):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, input_ents, ent_mask, labels=None):
        ...
        if labels is not None:
            ...
            return {'last_hidden_states': sequence_output, 'logits': prediction_scores, 'loss': masked_lm_loss}
        else:
            return prediction_scores
